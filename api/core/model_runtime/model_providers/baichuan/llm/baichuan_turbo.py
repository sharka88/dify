...
class BaichuanModel:
    ...
    def _build_parameters(
        self, model: str, stream: bool, messages: list[BaichuanMessage], parameters: dict[str, Any]
    ) -> dict[str, Any]:
        if (
            model == "baichuan2-turbo"
            or model == "baichuan2-turbo-192k"
            or model == "baichuan2-53b"
            or model == "baichuan3-turbo"
            or model == "baichuan3-turbo-128k"
            or model == "baichuan4"
        ):
            prompt_messages = []
    ...
    def generate(
        self, model: str, stream: bool, messages: list[BaichuanMessage], parameters: dict[str, Any], timeout: int
        ...
        try:
            response = post(url=api_base, headers=headers, data=dumps(data), timeout=timeout, stream=stream)
        except Exception as e:
